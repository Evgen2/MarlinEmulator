...
{   int rc;
    ...
    sock0 = socket( AF_INET, SOCK_STREAM,  0 /* SOCK_DGRAM = UDP SOCK_STREAM = TCP */ ); //создаем сокет
    if(sock0 == -1)
    {
#ifdef _WIN32
...
{   int rc;
    ...
    sock = socket( AF_INET, SOCK_STREAM,  0 /* SOCK_DGRAM = UDP SOCK_STREAM = TCP */ ); //создаем сокет
    if(sock == -1)
    {
#ifdef _WIN32
        printf("socket function failed with error = %d\n", WSAGetLastError() );
  ...
  {  
#ifdef _WIN32
//todo отдельно ругаться на ошибку WSAECONNREFUSED
	  int errcode;
