...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::_load(PORTARG_SOLO) {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      LOOP_XYZE_N(i) {
				if(!(i < XYZ + esteppers))
				{		planner.axis_steps_per_mm[i] = def1[i < COUNT(def1) ? i : COUNT(def1) - 1];
						planner.max_feedrate_mm_s[i] = def2[i < COUNT(def2) ? i : COUNT(def2) - 1];
						planner.max_acceleration_mm_per_s2[i] = def3[i < COUNT(def3) ? i : COUNT(def3) - 1];
				}
      }
#else					
