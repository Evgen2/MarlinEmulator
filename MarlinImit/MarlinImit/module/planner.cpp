...
float Planner::min_feedrate_mm_s,
  ...
  #endif
#else
  constexpr bool Planner::leveling_active=false;
#endif
...
void Planner::_buffer_steps(const int32_t (&target)[XYZE]
  ...
  if (block->steps[A_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[B_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[C_AXIS] < MIN_STEPS_PER_SEGMENT) {
    block->millimeters = FABS(delta_mm[E_AXIS]);
	  ...
	  block->millimeters =
	#if CORE_IS_XY
		  SQRT(sq(delta_mm[X_HEAD]) + sq(delta_mm[Y_HEAD]) + sq(delta_mm[Z_AXIS]));
	#elif CORE_IS_XZ
		  SQRT(sq(delta_mm[X_HEAD]) + sq(delta_mm[Y_AXIS]) + sq(delta_mm[Z_HEAD]));
	#elif CORE_IS_YZ
		  SQRT(sq(delta_mm[X_AXIS]) + sq(delta_mm[Y_HEAD]) + sq(delta_mm[Z_HEAD]));
	#else
