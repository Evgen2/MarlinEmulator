...
/***************************************************************/
 ...
 #ifndef USE_TMP03
#if defined(WIN32)
  static void* heater_ttbl_map[HOTENDS] = ARRAY_BY_HOTENDS((void*)HEATER,TEMPTABLE);
  static uint8_t heater_ttbllen_map[HOTENDS] = ARRAY_BY_HOTENDS(HEATER,TEMPTABLE_LEN);
#else
...
#if defined(WIN32)
int16_t Temperature::minttemp_raw[HOTENDS] = ARRAY_BY_HOTENDS(HEATER,RAW_LO_TEMP),
        Temperature::maxttemp_raw[HOTENDS] = ARRAY_BY_HOTENDS(HEATER,RAW_HI_TEMP),
#else
...
if(TMP03_idT2[1] > 1)
     celsius =	TMP03_CoeffSensorA[0] + TMP03_CoeffSensorB[0] *  float(TMP03_idT2[0]) / float(TMP03_idT2[1]);
...
if(TMP03_idT0[1] > 1)
     celsius =	TMP03_CoeffSensorA[0] + TMP03_CoeffSensorB[0] *  float(TMP03_idT0[0]) / float(TMP03_idT0[1]);
...
void Temperature::set_current_temp_raw() {
	...
	current_temperature_raw[0] = raw_temp_value[0]>>2;  //12->10 bit
#else
    ...
    #else
#if defined(__ARM_LPC1768__)
	current_temperature_raw[1] = raw_temp_value[1]>>2;//12->10 bit
#else
  ...
  #endif
#if defined(__ARM_LPC1768__)
		  current_temperature_bed_raw = raw_temp_bed_value>>2; //12->10 bit
#else
...
void Temperature::isr() {
  ...
  static void print_heater_state(const float &c, const float &t
    ...
    SERIAL_PROTOCOLCHAR_P(port, e == -1 ? 'B' : 'T');
      #elif HAS_TEMP_HOTEND
           SERIAL_PROTOCOLCHAR_P(port,'T');
	  #else
            SERIAL_PROTOCOLCHAR_P(port,'B');
      #endif
