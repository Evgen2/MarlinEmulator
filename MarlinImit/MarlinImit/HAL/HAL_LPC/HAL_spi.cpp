...
LPC_SSP_TypeDef  * p_LP_SSP;
#define LPC_SSP  p_LP_SSP            
  ...
  void spiBegin() {  // setup SCK, MOSI & MISO pins for SSP0		
    ...
    PinCfg.OpenDrain = 0;
    PinCfg.Pinmode = 0;
    PinCfg.Pinnum =  _PIN_NUM(SCK_PIN); // pin_map[SCK_PIN].pin;
    PinCfg.Portnum = _PIN_PORT(SCK_PIN); //pin_map[SCK_PIN].port;
    PINSEL_ConfigPin(&PinCfg);
//    SET_OUTPUT(SCK_PIN);

    PinCfg.Pinnum =  _PIN_NUM(MISO_PIN); //pin_map[MISO_PIN].pin;
    PinCfg.Portnum = _PIN_PORT(MISO_PIN); // pin_map[MISO_PIN].port;
    PINSEL_ConfigPin(&PinCfg);
  //  SET_INPUT(MISO_PIN);

    PinCfg.Pinnum =  _PIN_NUM(MOSI_PIN); // pin_map[MOSI_PIN].pin;
    PinCfg.Portnum = _PIN_PORT(MOSI_PIN); // pin_map[MOSI_PIN].port;
    PINSEL_ConfigPin(&PinCfg);
    // SET_OUTPUT(MOSI_PIN);
#if defined(USE_SSP0)
    LPC_SSP	 = LPC_SSP0;	
  ...
  void spiInit(uint8_t spiRate) {
   ...
   // setup for SPI mode
    ...
    SSP_Init(LPC_SSP, &HW_SPI_init);  // puts the values into the proper bits in the SSP0 registers

    SSP_Cmd(LPC_SSP, ENABLE);  // start SSP0 running
  }

  void spiSend(uint8_t b) {
    while (!SSP_GetStatus(LPC_SSP, SSP_STAT_TXFIFO_NOTFULL));   // wait for room in the buffer
    SSP_SendData(LPC_SSP, b & 0x00FF);
    while (SSP_GetStatus(LPC_SSP, SSP_STAT_BUSY));  // wait for it to finish
  }
  ...
  void spiSend(const uint8_t* buf, size_t n) {
    ...
    for (uint16_t i = 0; i < n; i++) {
      while (!SSP_GetStatus(LPC_SSP, SSP_STAT_TXFIFO_NOTFULL));   // wait for room in the buffer
      SSP_SendData(LPC_SSP, buf[i] & 0x00FF);
    }
    while (SSP_GetStatus(LPC_SSP, SSP_STAT_BUSY));  // wait for it to finish
  }
  ...
  uint8_t get_one_byte() {
   // send a dummy byte so can clock in receive data
    SSP_SendData(LPC_SSP,0x00FF);
    while (SSP_GetStatus(LPC_SSP, SSP_STAT_BUSY));  // wait for it to finish
    return SSP_ReceiveData(LPC_SSP) & 0x00FF;
  }

  // Read single byte from SPI
  uint8_t spiRec() {
    while (SSP_GetStatus(LPC_SSP, SSP_STAT_RXFIFO_NOTEMPTY) || SSP_GetStatus(LPC_SSP, SSP_STAT_BUSY))
				SSP_ReceiveData(LPC_SSP);  //flush the receive buffer
    return get_one_byte();
  }
