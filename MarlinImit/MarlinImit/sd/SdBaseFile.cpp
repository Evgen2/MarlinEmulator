...
bool SdBaseFile::addDirCluster() {
  uint32_t block;
  // max folder size
  if (fileSize_ / sizeof(dir_t) >= 0xFFFF) goto _FAIL;

  if (!addCluster()) goto _FAIL;
  if (!vol_->cacheFlush()) goto _FAIL;
  ...
  for (uint8_t i = 1; i < vol_->blocksPerCluster_; i++) {
    if (!vol_->writeBlock(block + i, vol_->cacheBuffer_.data)) goto _FAIL;
  }
...
dir_t* SdBaseFile::cacheDirEntry(uint8_t action) {
  if (!vol_->cacheRawBlock(dirBlock_, action)) goto _FAIL;
...
bool SdBaseFile::contiguousRange(uint32_t* bgnBlock, uint32_t* endBlock) {
  // error if no blocks
  if (firstCluster_ == 0) goto _FAIL;

  for (uint32_t c = firstCluster_; ; c++) {
    uint32_t next;
    if (!vol_->fatGet(c, &next)) goto _FAIL;

    // check for contiguous
    if (next != (c + 1)) {
      // error if not end of chain
      if (!vol_->isEOC(next)) goto _FAIL;
...
bool SdBaseFile::createContiguous(SdBaseFile* dirFile,
  ...
  if (size == 0) goto _FAIL;
  if (!open(dirFile, path, O_CREAT | O_EXCL | O_RDWR)) goto _FAIL;
...
int8_t SdBaseFile::lsPrintNext(uint8_t flags, uint8_t indent) {
  ...
  for (uint8_t i = 0; i < indent; i++) MYSERIAL.write_0(' ');
...
bool SdBaseFile::make83Name(const char* str, uint8_t* name, const char** ptr) {
  ...
  while (*str != '\0' && *str != '/') {
    c = *str++;
    if (c == '.') {
      if (n == 10) goto _FAIL;  // only one dot allowed
      ...
      PGM_P p = PSTR("|<>^+=?/[];,*\"\\");
      uint8_t b;
      while ((b = pgm_read_byte(p++))) if (b == c) goto _FAIL;
      // check size and only allow ASCII printable characters
      if (i > n || c < 0x21 || c == 0x7F) goto _FAIL;
...
bool SdBaseFile::mkdir(SdBaseFile* parent, const char* path, bool pFlag) {
  ...
  if (!parent || isOpen()) goto _FAIL;

  if (*path == '/') {
    while (*path == '/') path++;
    if (!parent->isRoot()) {
      if (!dir2.openRoot(parent->vol_)) goto _FAIL;
      parent = &dir2;
    }
  }
  while (1) {
    if (!make83Name(path, dname, &path)) goto _FAIL;
    while (*path == '/') path++;
    if (!*path) break;
...
bool SdBaseFile::mkdir(SdBaseFile* parent, const uint8_t dname[11]) {
  ...
  if (!parent->isDir()) goto _FAIL;

  // create a normal file
  if (!open(parent, dname, O_CREAT | O_EXCL | O_RDWR)) goto _FAIL;
  ...
  if (!vol_->cacheRawBlock(block, SdVolume::CACHE_FOR_WRITE)) goto _FAIL;
...
bool SdBaseFile::open(SdBaseFile* dirFile, const char* path, uint8_t oflag) {
  ...
  if (!dirFile) goto _FAIL;

  // error if already open
  if (isOpen()) goto _FAIL;

  if (*path == '/') {
    while (*path == '/') path++;
    if (!dirFile->isRoot()) {
      if (!dir2.openRoot(dirFile->vol_)) goto _FAIL;
      parent = &dir2;
    }
  }
  while (1) {
    ...
    while (*path == '/') path++;
    if (!*path) break;
    if (!sub->open(parent, dname, O_READ)) goto _FAIL;
...
bool SdBaseFile::open(SdBaseFile* dirFile,
  ...
  if (fileFound) {
    ...
    if (oflag & O_EXCL) goto _FAIL;
  }
  else {
    // don't create unless O_CREAT and O_WRITE
    if (!(oflag & O_CREAT) || !(oflag & O_WRITE)) goto _FAIL;
    if (emptyFound) {
      index = dirIndex_;
      p = cacheDirEntry(SdVolume::CACHE_FOR_WRITE);
      if (!p) goto _FAIL;
      ...
      if (dirFile->type_ == FAT_FILE_TYPE_ROOT_FIXED) goto _FAIL;

      // add and zero cluster for dirFile - first cluster is in cache for write
      if (!dirFile->addDirCluster()) goto _FAIL;
    ...
    // write entry to SD
    if (!dirFile->vol_->cacheFlush()) goto _FAIL;
  }
  // open entry in cache
...
bool SdBaseFile::open(SdBaseFile* dirFile, uint16_t index, uint8_t oflag) {
  ...
  // error if already open
  if (isOpen() || !dirFile) goto _FAIL;

  // don't open existing file if O_EXCL - user call error
  if (oflag & O_EXCL) goto _FAIL;

  // seek to location of entry
  if (!dirFile->seekSet(32 * index)) goto _FAIL;
...
bool SdBaseFile::openCachedEntry(uint8_t dirIndex, uint8_t oflag) {
  ...
  if (p->attributes & (DIR_ATT_READ_ONLY | DIR_ATT_DIRECTORY)) {
    if (oflag & (O_WRITE | O_TRUNC)) goto _FAIL;
  }
  ...
  if (DIR_IS_FILE(p)) {
    ...
    if (!vol_->chainSize(firstCluster_, &fileSize_)) goto _FAIL;
    type_ = FAT_FILE_TYPE_SUBDIR;
  }
  else {
    goto _FAIL;
...
bool SdBaseFile::openNext(SdBaseFile* dirFile, uint8_t oflag) {
  ...
  while (1) {
    ...
    // done if last entry
    if (p->name[0] == DIR_NAME_FREE) goto _FAIL;
...
bool SdBaseFile::openParent(SdBaseFile* dir) {
  ...
  if (isOpen() || !dir || dir->isRoot() || !dir->isDir()) goto _FAIL;
  vol_ = dir->vol_;
  // position to '..'
  if (!dir->seekSet(32)) goto _FAIL;
  // read '..' entry
  if (dir->read(&entry, sizeof(entry)) != 32) goto _FAIL;
  // verify it is '..'
  if (entry.name[0] != '.' || entry.name[1] != '.') goto _FAIL;
  // start cluster for '..'
  ...
  // verify name for '../..'
  if (p->name[0] != '.' || p->name[1] != '.') goto _FAIL;
  // '..' is pointer to first cluster of parent. open '../..' to find parent
  if (p->firstClusterHigh == 0 && p->firstClusterLow == 0) {
    if (!file.openRoot(dir->volume())) goto _FAIL;
  }
  ...
  do {
    if (file.readDir(&entry, NULL) != 32) goto _FAIL;
...
bool SdBaseFile::openRoot(SdVolume* vol) {
  ...
  if (vol->fatType() == 16 || (FAT12_SUPPORT && vol->fatType() == 12)) {
    ...
    if (!vol->chainSize(firstCluster_, &fileSize_)) goto _FAIL;
  }
  else {
...
int16_t SdBaseFile::read(void* buf, uint16_t nbyte) {
  ...
  if (!isOpen() || !(flags_ & O_READ)) goto _FAIL;
  ...
  while (toRead > 0) {
    ...
    if (type_ == FAT_FILE_TYPE_ROOT_FIXED) {
      ...
      if (offset == 0 && blockOfCluster == 0) {
        ...
        if (curPosition_ == 0) {
          ...
          if (!vol_->fatGet(curCluster_, &curCluster_)) goto _FAIL;
        }
      }
    ...
    if (n == 512 && block != vol_->cacheBlockNumber()) {
      if (!vol_->readBlock(block, dst)) goto _FAIL;
    }
    else {
      // read block to cache and copy data to caller
      if (!vol_->cacheRawBlock(block, SdVolume::CACHE_FOR_READ)) goto _FAIL;
...
bool SdBaseFile::remove(SdBaseFile* dirFile, const char* path) {
  SdBaseFile file;
  if (!file.open(dirFile, path, O_WRITE)) goto _FAIL;
  return file.remove();
  _FAIL:
...
bool SdBaseFile::rename(SdBaseFile* dirFile, const char* newPath) {
  ...
  if (!(isFile() || isSubDir())) goto _FAIL;

  // can't move file
  if (vol_ != dirFile->vol_) goto _FAIL;
  ...
  if (dirCluster) {
    ...
    // free unused cluster
    if (!vol_->freeChain(dirCluster)) goto _FAIL;
...
bool SdBaseFile::rmdir() {
  ...
  while (curPosition_ < fileSize_) {
    ...
    // error not empty
    if (DIR_IS_FILE_OR_SUBDIR(p)) goto _FAIL;
  }
...
bool SdBaseFile::rmRfStar() {
  ...
  while (curPosition_ < fileSize_) {
    ...
    if (!f.open(this, index, O_READ)) goto _FAIL;
    if (f.isSubDir()) {
      // recursively delete
      if (!f.rmRfStar()) goto _FAIL;
    }
    else {
      // ignore read-only
      f.flags_ |= O_WRITE;
      if (!f.remove()) goto _FAIL;
    }
    // position to next entry if required
    if (curPosition_ != (32 * (index + 1))) {
      if (!seekSet(32 * (index + 1))) goto _FAIL;
    }
  }
...
bool SdBaseFile::seekSet(uint32_t pos) {
  ...
  if (!isOpen() || pos > fileSize_) goto _FAIL;
...
bool SdBaseFile::sync() {
  ...
  if (flags_ & F_FILE_DIR_DIRTY) {
    ...
    if (!d || d->name[0] == DIR_NAME_DELETED) goto _FAIL;
...
bool SdBaseFile::timestamp(SdBaseFile* file) {
  ...
  // get timestamps
  if (!file->dirEntry(&dir)) goto _FAIL;
...
bool SdBaseFile::truncate(uint32_t length) {
  ...
  if (!isFile() || !(flags_ & O_WRITE)) goto _FAIL;

  // error if length is greater than current size
  if (length > fileSize_) goto _FAIL;
  ...
  if (!seekSet(length)) goto _FAIL;

  if (length == 0) {
    // free all clusters
    if (!vol_->freeChain(firstCluster_)) goto _FAIL;
    firstCluster_ = 0;
  }
  else {
    uint32_t toFree;
    if (!vol_->fatGet(curCluster_, &toFree)) goto _FAIL;

    if (!vol_->isEOC(toFree)) {
      // free extra clusters
      if (!vol_->freeChain(toFree)) goto _FAIL;

      // current cluster is end of chain
      if (!vol_->fatPutEOC(curCluster_)) goto _FAIL;
    }
  }
  fileSize_ = length;
  ...
  while (nToWrite > 0) {
    ...
    if (blockOfCluster == 0 && blockOffset == 0) {
      ...
      if (curCluster_ == 0) {
        ...
        uint32_t next;
        if (!vol_->fatGet(curCluster_, &next)) goto _FAIL;
        if (vol_->isEOC(next)) {
    ...
    if (n == 512) {
      ...
      if (!vol_->writeBlock(block, src)) goto _FAIL;
    }
    else {
